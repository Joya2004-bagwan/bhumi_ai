...
async def download_batch(batch_id: str, format: str = "json"):
    ...
    buf.seek(0)
    for item in done_items:
        log_event("export", image_id=item.item_id, filename=item.original_filename,
                  export_format=format, building_count=item.building_count,
                  model=job.model, source="batch")
