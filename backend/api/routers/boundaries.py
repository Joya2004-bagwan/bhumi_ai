...
async def detect_boundaries(request: BoundaryRequest):
    ...
    try:
        ...
        log_event("boundary_detect", image_id=request.image_id, model=request.model,
                  building_count=len(buildings), processing_time=round(processing_time, 3))
...
async def gpt_boundaries(request: GPTBoundaryRequest):
    ...
    try:
        ...
        log_event("boundary_detect", image_id=request.image_id, model=request.model,
                  building_count=len(result["polygons"]), processing_time=round(processing_time, 2))
