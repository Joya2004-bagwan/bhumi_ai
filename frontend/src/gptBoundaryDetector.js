...
export class GPTBoundaryDetector {
  ...
  _download(blob, filename) {
    ...
    // log the export to the backend so the dashboard picks it up
    ...
    fetch('/api/log-export', {
      ...
      body: JSON.stringify({
        filename,
        export_format: ext,
        image_id: this.imageUploader?.imageId || '',
        building_count: this._polygons?.length || 0,
        model: this.modelSelector?.getSelectedModel() || '',
      }),
    }).catch(() => {});
  }
